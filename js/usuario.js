...
function cancelarUsr() {
    ...
    document.getElementById("txtEmUsr").value = "";
}
...
function agregarUsr() {
    ...
    let mail = document.getElementById("txtEmUsr").value;
    let foto = document.getElementById("txtFotoUsr").value;

    var myHeaders = new Headers();
    myHeaders.append("apikey", "cH6PMjKsknA9HdGP7ri6QyQbgGCwviIl");

    let mailTo = encodeURIComponent(mail);
    var requestOptions = {
        method: 'GET',
        redirect: 'follow',
        headers: myHeaders
    };
    fetch(`https://api.apilayer.com/email_verification/check?email=${mailTo}`, requestOptions)
            .then(response => response.json())  // Parse response as JSON
            .then(result => {
                // Check if 'disposable' is false and 'format_valid' is true
                if (result.disposable === false && result.format_valid === true) {
                    console.log("The email is not disposable and the format is valid.");
                    ...
                    console.log("Either the email is disposable or the format is invalid.");
                    Swal.fire({
                        title: "Inserción incorrecta",
                        text: "El usuario no ha sido insertado correctamente",
                        icon: "error"
                    });
                }
            }
