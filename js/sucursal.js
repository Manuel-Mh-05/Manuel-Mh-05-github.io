...
function cargarCatalogo() {
    ...
    imprimirTablaSuc();
    map = L.map('mapa').setView([listaSuc.sucursales[i].latitud, listaSuc.sucursales[i].longitud], 13); 

  
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        attribution: '&copy; OpenStreetMap contributors'
    }).addTo(map);
    marker = L.marker([listaSuc.sucursales[i].latitud, listaSuc.sucursales[i].longitud]).addTo(map)
        .bindPopup('Sucursal ' + (i+1) + ': ' + listaSuc.sucursales[i].latitud + ', ' + listaSuc.sucursales[i].longitud)
        .openPopup();
}
...
function modificarSuc(i) {
    ...
    let latitude = listaSuc.sucursales[i].latitud;
    let longitude = listaSuc.sucursales[i].longitud;
    ...
    marker = L.marker([latitude, longitude]).addTo(map)
        .bindPopup('Sucursal ' + (i+1) + ': '+ listaSuc.sucursales[i].direccion)
        .openPopup();
}

function modSuc(i) {
    listaSuc.sucursales[i].id = document.getElementById("txtIdSuc").value;
    listaSuc.sucursales[i].nom = document.getElementById("txtNomSuc").value;
    listaSuc.sucursales[i].dir = document.getElementById("txtDirSuc").value;
    listaSuc.sucursales[i].hor = document.getElementById("txtHorSuc").value;
    listaSuc.sucursales[i].lat = document.getElementById("txtLatSuc").value;
    listaSuc.sucursales[i].lng = document.getElementById("txtLngSuc").value;
    listaSuc.sucursales[i].est = document.getElementById("txtEstSuc").value;
    listaSuc.sucursales[i].foto = document.getElementById("txtFotoSuc").value;

    let sucursal = {"id": listaSuc.sucursales[i].id, "nombre": listaSuc.sucursales[i].nom,
        "direccion": listaSuc.sucursales[i].dir, "horario": listaSuc.sucursales[i].hor,
        "latitud": listaSuc.sucursales[i].lat, "longitud": listaSuc.sucursales[i].lng,
        "estatus": listaSuc.sucursales[i].est, "foto": listaSuc.sucursales[i].foto};

    listaSuc.sucursales[i] = sucursal;
...
function agregarSuc() {
    ...
    let lng = document.getElementById("txtLngSuc").value;
    let est = document.getElementById("txtEstSuc").value;
    let foto = document.getElementById("txtFotoSuc").value;
    let sucursal = {"id": id, "nombre": nom, "direccion": dir,
        "horario": hor, "latitud": lat, "longitud": lng, "estatus": est, "foto": foto};
